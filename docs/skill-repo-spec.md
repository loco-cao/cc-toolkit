...
一个符合 AIT 规范的 GitHub 仓库必须遵循以下约定，以便 `ait register` 自动发现和安装。

## Repository Layout

```
<repo-name>/
├── ait.yaml                 # 必需：skill 元数据（仓库根目录）
├── claude/
...
## ait.yaml Specification

### Required Fields

```yaml
name: adsense-lint                    # 唯一标识（kebab-case）
...
targets:                              # 至少一个 CLI 目标
  claude:
    trigger: /adsense-lint            # 在 CLI 中触发的命令
    path: claude/SKILL.md             # 相对仓库根的 skill 文件路径
  codex:
    trigger: /adsense-lint
    path: codex/SKILL.md
```
...
output_dir: .adsense-lint              # 运行时输出目录（默认 .adsense-lint）
...
```
<required>    → 必填参数，ait 弹出输入框
[--optional]  → 可选参数，ait 弹出开关
<url>         → 特殊类型，ait 用 URL 验证
```
...
targets:
  claude:
    trigger: /adsense-lint
  codex:
    trigger: /skills adsense-lint     # Codex 用不同语法
```
...
## Example: adsense-lint

仓库 `github.com/lococao/adsense-lint`：
