const path = require('path');
const os = require('os');
const {
  AgentState,
  cleanOutput,
  DEFAULT_TRUST_PATTERNS,
  DEFAULT_PERMISSION_PATTERNS,
  createAnswerer,
  createStateDetector,
} = require('./automation.js');

const ADAPTER_NAME = 'claude';
const DISPLAY_NAME = 'Claude Code';

// ── CLI-specific patterns ──────────────────────────────────────────
// Claude Code trust prompt: "Yes, and don't ask again" with "Tab to amend / Esc to cancel"
// The reply "2\r" selects the "Yes, don't ask again" option.
const CLAUDE_TRUST_PATTERNS = [
  ...DEFAULT_TRUST_PATTERNS,
  "yes, and don't ask again",
  'yes, don\'t ask again',
  'tab to amend',
  'esc to cancel',
];

// Claude Code uses the same keyboard-navigation dialog for permissions
const CLAUDE_PERM_PATTERNS = [
  ...DEFAULT_PERMISSION_PATTERNS,
  "yes, and don't ask again",
  'yes, don\'t ask again',
  'tab to amend',
  'esc to cancel',
  'do you want to proceed',
];

// Build answerers
const _answerTrust = createAnswerer(CLAUDE_TRUST_PATTERNS, '2\r');
const _answerPermission = createAnswerer(CLAUDE_PERM_PATTERNS, '2\r');
const _detectState = createStateDetector(CLAUDE_TRUST_PATTERNS, CLAUDE_PERM_PATTERNS);

// ── Public adapter ─────────────────────────────────────────────────

function detect() {
  const { execSync } = require('child_process');
  try {
    const isWin = process.platform === 'win32';
    const binCmd = isWin ? 'where claude 2>nul' : 'which claude 2>/dev/null';
    const out = execSync(binCmd, { shell: true, encoding: 'utf8', timeout: 5000 });
    const lines = out.trim().split(/\r?\n/).filter(Boolean);

    let best = null;
    for (const raw of lines) {
      const cleaned = raw.trim();
      if (!cleaned) continue;
      try { require('fs').accessSync(cleaned); } catch (_) { continue; }
      if (isWin) {
        if (cleaned.endsWith('.exe')) return cleaned;
        if (cleaned.endsWith('.cmd') && !best) best = cleaned;
      } else {
        return cleaned;
      }
    }
    return best;
  } catch (_) {
    return null;
  }
}

function getVersion(bin) {
  try {
    const { execSync } = require('child_process');
    return execSync(`"${bin}" --version`, { encoding: 'utf8', timeout: 10000 }).trim();
  } catch (_) {
    return 'unknown';
  }
}

function spawn(bin, cwd) {
  let pty;
  try {
    pty = require('node-pty');
  } catch (_) {
    return null;
  }

  return pty.spawn(bin, [], {
    name: 'xterm-color',
    cols: 120,
    rows: 30,
    cwd,
    env: process.env,
  });
}

function answerTrust(output) {
  return _answerTrust(output);
}

function answerPermission(output) {
  return _answerPermission(output);
}

function detectState(output) {
  return _detectState(output);
}

function formatCommand(skillName, args) {
  const argStr = args.length > 0 ? ' ' + args.join(' ') : '';
  return `/${skillName}${argStr}\r`;
}

function getInstallPaths() {
  const home = os.homedir();
  return {
    skills: path.join(home, '.claude', 'skills'),
    agents: path.join(home, '.claude', 'agents'),
  };
}

module.exports = {
  name: ADAPTER_NAME,
  displayName: DISPLAY_NAME,
  detect,
  getVersion,
  spawn,
  answerTrust,
  answerPermission,
  detectState,
  formatCommand,
  getInstallPaths,
};
