...
class AgentRunner {
  constructor() {
    ...
    this.traceLines = [];
    this._nextColor = 0;
    this.skillReport = null;   // populated from skill-produced summary report if present
  }
  ...
  syncAgent(id) {
    ...
    this.agents.push({ id, name, status: 'waiting', message: '', score: null, color: this._assignColor(), heartbeat: null });
  }
  ...
  render() {
    ...
    const header = `${ANSI.bold}CCT — Skill Runner${ANSI.reset}`;
    lines.push('');
    lines.push(`  ${header}`);
    lines.push(`  ${colorize('═'.repeat(42), ANSI.gray)}`);

    // Sort: running → waiting → done/failed → skipped
    const order = { running: 0, waiting: 1, done: 2, failed: 3, skipped: 4 };
  ...
  /** Build a generic score table. Skill-specific interpretation comes from the skill's summary report if present. */
  summary() {
    ...
    if (this.skillReport) {
      result.skillReport = this.skillReport;
    }
 ...
 * @param {string} options.cli          - 'auto' | 'claude'
 * @param {string} options.cwd          - working directory
 * @param {string} options.outputDir    - from cct.yaml (e.g. '.cct-skill')
 * @param {number} options.globalTimeout - from cct.yaml (seconds), converted to ms
 */
async function runSkill(skillName, args, options = {}) {
  ...
  // ── Skill output directory (from cct.yaml, default '.cct-skill') ──
  const outputDirName = options.outputDir || '.cct-skill';
  ...
  return new Promise((resolve) => {
    ...
    const globalTimeout = options.globalTimeout
      ? options.globalTimeout * 1000    // cct.yaml uses seconds
      : 600_000;                        // default 10 min
    const STABILIZATION_MS = 30_000;    // no new reports + PTY idle for 30s → skill done
    const SUMMARY_GRACE_MS = 60_000;    // wait up to 60s for summary report after stabilization
    const MIN_RUN_MS = 120_000;         // minimum execution before stabilization can trigger
    let resolved = false;
    let ptyExited = false;
    let reportCount = 0;
    ...
    let lastReportTime = 0;             // when the most recent report was found
    let lastPtyOutputTime = 0;          // when the PTY last produced output
    ...
    const finish = () => {
      ...
      resolved = true;
      runner.stopSpinner();
      // Mark agents still waiting/running as skipped
      for (const agent of runner.agents) {
        if (agent.status !== 'done' && agent.status !== 'failed') {
          runner.setStatus(agent.id, 'skipped', 'no report');
        }
      }
      runner.render();
    ...
    const check = setInterval(() => {
      ...
      if (!commandSent && !ptyExited && elapsed >= 5000) {
        ...
        if (!stillPrompting || elapsed >= 12000) {
          ...
          lastPtyOutputTime = Date.now();  // start PTY idle clock
      ...
      if (sessionDir && fs.existsSync(sessionDir)) {
        ...
        for (const agent of runner.agents) {
          ...
          // Read heartbeat status.json for real-time progress
          if (agent.status === 'running') {
            const statusPath = path.join(sessionDir, agent.id, 'status.json');
            if (fs.existsSync(statusPath)) {
              try {
                const hb = JSON.parse(fs.readFileSync(statusPath, 'utf8'));
                if (hb && hb.step) {
                  agent.heartbeat = hb;
                  const pct = hb.progress != null ? ` ${Math.round(hb.progress * 100)}%` : '';
                  agent.message = `${hb.step}${pct} — ${hb.message || ''}`;
                }
              } catch (_) {}
            }
          }
        ...
        // Try to load a skill-produced summary report (any dir named *-summary or summary)
        if (!runner.skillReport) {
          for (const id of dirIds) {
            if (/summary/.test(id)) {
              const sp = path.join(sessionDir, id, 'report.json');
              if (fs.existsSync(sp)) {
                try {
                  runner.skillReport = JSON.parse(fs.readFileSync(sp, 'utf8'));
                } catch (_) {}
              }
              break;
            }
      ...
      const noNewReports = lastReportTime > 0 && (Date.now() - lastReportTime) >= STABILIZATION_MS;
      const ptyIdle = lastPtyOutputTime > 0 && (Date.now() - lastPtyOutputTime) >= STABILIZATION_MS;
      const hasReports = reportCount > 0;
      const pastMinRun = elapsed >= MIN_RUN_MS;
      // PTY must be idle AND no new reports for stabilization to trigger.
      // This prevents early exit while agents are still working but haven't written reports yet.
      const stabilized = pastMinRun && hasReports && noNewReports && ptyIdle && noNewAgents;

      // ── Completion: PTY idle + report stabilization ──
      // Skill is done when the CLI stops producing output AND no new reports appear.
      if (stabilized && !timedOut) {
        ...
        // Grace period after exit for skill summary report to flush
        if (!stabilizeStart) stabilizeStart = Date.now();
        if (Date.now() - stabilizeStart >= SUMMARY_GRACE_MS) {
          clearInterval(check);
      ...
      // ── PTY exited early ──
      if (ptyExited && hasReports && noNewReports && noNewAgents) {
        clearInterval(check);
        finish();
      }
    }, 1500);
