...
async function runSkill(skillName, args, options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const STABILIZATION_MS = 30_000;    // no new reports/agents for 30s → skill done
    ...
    const check = setInterval(() => {
      ...
      const stabilized = hasReports && noNewReports && noNewAgents;

      // ── Completion: report-based stabilization ──
      // Skill is done when no new reports AND no new agents for STABILIZATION_MS.
      // Agents that never produce a report (e.g. a summary dir placeholder) don't block completion.
