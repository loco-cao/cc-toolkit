...
function printBanner() {
  ...
  console.log(`  ${colorize('║', ANSI.cyan)}     ${ANSI.bold}CCT — CC Toolkit · Universal Skill Launcher${ANSI.reset}        ${colorize('║', ANSI.cyan)}`);
  console.log(`  ${colorize('╚══════════════════════════════════════════════════════╝', ANSI.cyan)}`);
  console.log('');
}
...
async function selectSkill(registrations) {
  if (registrations.length === 0) {
    console.log(`  ${colorize('No registered skills. Use "cct register gh:user/repo" first.', ANSI.yellow)}`);
    console.log('');
    return null;
  }
...
async function askScope() {
  return select({
    ...
    choices: [
      {
        name: ` ${colorize('Global', ANSI.brightCyan)}  → ~/.cct/repos/  (available everywhere)`,
        value: 'global',
      },
      {
        name: ` ${colorize('Project', ANSI.brightBlue)} → ./.cct/repos/  (this project only)`,
        value: 'project',
      },
    ],
    loop: false,
